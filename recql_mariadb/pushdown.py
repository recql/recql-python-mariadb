"""MariaDB prefilter pushdown capability matrix."""
...
MARIADB_PUSHDOWN: dict[str, PushdownCapability] = {
    "filter": PushdownCapability(
        "filter",
        frozenset({PrefilterShape.ARBITRARY}),
        "Full SQL WHERE fragment (trust engine-authored predicates).",
    ),
    ...
    "column_order": PushdownCapability(
        ...
        "Equality / IN / AND-OR only; ranges and functions fail closed.",
    ),
...
def supports_prefilter(retriever_type: str, expr: A.Expr | str | None) -> bool:
    return _supports(MARIADB_PUSHDOWN, retriever_type, expr)


def assert_pushdown_or_raise(retriever_type: str, expr: A.Expr | str | None) -> None:
    _assert(MARIADB_PUSHDOWN, retriever_type, expr)
